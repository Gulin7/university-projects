...
def remove_contestant(all_contestants, *args):
    ...
    """
    if len(args) == 1:
        all_contestants.pop(args[0] - 1)
    elif len(args) == 2:
        try:
            score = int(args[1])
            if args[0] == '=':
                for contestant in all_contestants:
                    total = int((contestant["p1"] + contestant["p2"] + contestant["p3"]))
                    if (total / 3) == score:
                        contestant["p1"] = 0
                        contestant["p2"] = 0
                        contestant["p3"] = 0
        ...
        if args[1].lower() != 'to':
            ...
            for i in range(args[0] - 1, args[2]):
                all_contestants.pop(args[0] - 1)
    else:
        print("Invalid command!")
...
def print_all(all_contestants, *args):
    ...
    """
    if len(args) < 1:
        print(f"\nThe contestants are: {all_contestants}! \n")
    elif len(args) == 1:
        if args[0] != 'sorted':
            print("Invalid command!")
        else:
            print(f"\nThe contestants in sorted order are: {operations.sort_contestants(all_contestants)}! \n")
    else:
        printed = 0
        try:
            score = int(args[1])
...
def run_console():
    ...
    commands = {
        ...
        "undo": operations.undo_operations,
    }
