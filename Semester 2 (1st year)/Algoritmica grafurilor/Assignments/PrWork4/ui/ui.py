...
0️⃣ Exit. """)
    ...
    def runUi(self):
        ...
        graph = readGraphFromFile2('minimumTree1')
        while True:
            self.showMenu()
...
The out degree is: {outDegree}""")
                ...
                tree = graph.primAlg(startVertex)
                for edgeWCost in tree.getAllEdgesCosts():
                    edge = edgeWCost[0]
                    cost = edgeWCost[1]
                    print(f"Edge: {edge[0]} {edge[1]} | Cost: {cost}")
            elif command == 18:
                pass
