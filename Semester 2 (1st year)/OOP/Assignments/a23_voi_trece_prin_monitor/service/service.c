...
int updateInService(Service *service, char *name, int concentration, int quantity, double price) {
    Medicine *medicineToUpdate = createMedicine(name, concentration, quantity, price);
    Medicine *outcome = updateRepository(service->repository, medicineToUpdate);
    if (outcome != NULL) {
...
int undoOperation(Service *service) {
    ...
    if (strcmp(getOperationType(operation), "add") == 0) {
        ...
        strcpy(redoOperation->operationType, "add");
        Medicine *medicine = copyMedicine(getOperationMedicine(operation));
        addToRepository(service->repository, medicine);
        destroyMedicine(medicine);
    } else if (strcmp(getOperationType(operation), "update") == 0) {
        Medicine *medicine = copyMedicine(getOperationMedicine(operation));
        redoOperation->medicine = copyMedicine(updateRepository(service->repository, medicine));
        destroyMedicine(medicine);
    }

    pushOperation(service->redoStack, redoOperation);
...
int redoOperation(Service *service) {
    ...
    Operation *operation = popOperation(service->redoStack);
    Operation *undoOperation = operationCopy(operation);

    if (strcmp(getOperationType(operation), "add") == 0) {
        strcpy(undoOperation->operationType, "del");
        Medicine *medicine = copyMedicine(getOperationMedicine(operation));
        deleteFromRepository(service->repository, medicine->name, medicine->concentration);
        destroyMedicine(medicine);
    } else if (strcmp(getOperationType(operation), "del") == 0) {
        strcpy(undoOperation->operationType, "add");
        Medicine *medicine = copyMedicine(getOperationMedicine(operation));
        addToRepository(service->repository, medicine);
        destroyMedicine(medicine);
    } else if (strcmp(getOperationType(operation), "update") == 0) {
        Medicine *medicine = copyMedicine(getOperationMedicine(operation));
        undoOperation->medicine = copyMedicine(updateRepository(service->repository, medicine));
        destroyMedicine(medicine);
    }

    pushOperation(service->undoStack, undoOperation);
