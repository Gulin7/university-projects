...
{
	return "Title: " + this->getTitle() + " | Description: " + this->getDescription() + " \n| Year: " + std::to_string(this->getDate().year) +
		" | Month: " + std::to_string(this->getDate().month) + " | Day: " + std::to_string(this->getDate().day) +
		" \n| Minute: " + std::to_string(this->getTime().minute) + " | Hour: " + std::to_string(this->getTime().hour) +
		" \n| Number of people: " + std::to_string(this->getNumberOfPeople()) + " | Link: " + this->getLink();
}
...
std::istream& operator>>(std::istream& inputFile, Event& eventToRead)
{
	...
	if (dateTokens.size() != 3)
		return inputFile;
	eventToRead.date = Date{ std::stoi(dateTokens[2]), std::stoi(dateTokens[1]), std::stoi(dateTokens[0]) };
