...
{
	std::vector <Event> events = this->userService.getEventList()->getAllEvents();
	int index = 1;
	for (const auto& event : events) {
		std::cout << "#" << index << std::endl;
		index++;
...
void Ui::showUserMenu()
{
	...
	std::cout << "4. SAVE event list \n";
	std::cout << "5. Open event list \n";
	std::cout << "0. Exist user mode \n";
}

void Ui::saveEventlist()
{
	std::string filename;
	std::cout << "Enter a filename (path): ";
	std::cin.ignore();
	getline(std::cin, filename);
	this->userService.saveEventlist();

	if (this->userService.getEventList() == nullptr) {
		std::cout << "Can't save!\n";
		return;
	}
}

void Ui::openEventlist()
{
