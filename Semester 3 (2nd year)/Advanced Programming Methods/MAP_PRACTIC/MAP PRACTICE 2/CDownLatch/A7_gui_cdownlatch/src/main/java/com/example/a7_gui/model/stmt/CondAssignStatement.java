...
public class CondAssignStatement implements IStatement {
    ...
    public ProgramState execute(ProgramState state) throws StatementExecutionException, ExpressionEvaluationException, ADTException, DivisionByZero {
        ...
        // if expr1 is true, it will assign expr2 to variable, otherwise it will assign expr3 to variable
        exeStack.push(ifStatement); //push onto the stack the if statement
        state.setExeStack(exeStack); //sets the states execution stack to the new stack
    ...
    public MyIDictionary<String, Type> typeCheck(MyIDictionary<String, Type> typeEnv) throws StatementExecutionException, ExpressionEvaluationException, ADTException {
        ...
        //checks if expr1 is a boolean, and if expr2 and expr3 are of the same type as the variable
        if(type1.equals(new BoolType()) && type2.equals(varType) && type3.equals(varType))
            return typeEnv;
        else throw new StatementExecutionException("CondAssignStmt: One of the expressions has no compatible type");
    }
