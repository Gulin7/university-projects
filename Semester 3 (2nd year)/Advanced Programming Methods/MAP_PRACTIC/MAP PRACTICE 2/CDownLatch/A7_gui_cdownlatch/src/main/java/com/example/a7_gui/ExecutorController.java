...
public class ExecutorController {
    ...
    @FXML
    private TableView<Pair<Integer,Integer>> countDownLatchTableView;
    @FXML
    private TableColumn<Pair<Integer, Integer>, Integer> latchAddress;
    @FXML
    ...
    public void initialize()
    {
        ...
        latchAddress.setCellValueFactory(p -> new SimpleIntegerProperty(p.getValue().first).asObject());
        latchValue.setCellValueFactory(p -> new SimpleIntegerProperty(p.getValue().second).asObject());
    }
    ...
    private void populateCountdownLatchTableView() {
        ProgramState programState = getCurrentProgramState();
        MyICountdownLatchTable countdownLatchTable = Objects.requireNonNull(programState).getCountdownLatchTable();

        ArrayList<Pair<Integer, Integer>> latchEntries = new ArrayList<>();

        for(Map.Entry<Integer, Integer> entry : countdownLatchTable.getCountdownLatchTable().entrySet())
            latchEntries.add(new Pair<>(entry.getKey(), entry.getValue()));

        countDownLatchTableView.setItems(FXCollections.observableArrayList(latchEntries));
    }
